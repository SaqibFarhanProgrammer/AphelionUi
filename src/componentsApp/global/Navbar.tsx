...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-5">
      ...
      <div className="flex items-center justify-between h-[52px] px-2 pr-2 pl-5 bg-[#0A0A0A]/90 backdrop-blur-xl rounded-full border border-white/[0.08] shadow-[0_8px_32px_rgba(0,0,0,0.4)] max-w-[70%] w-full mx-4">
        {/* Logo */}
        <a
          href="/"
        ...
        <div className="hidden md:flex items-center gap-0.5 mx-4">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="px-3 py-1.5 text-[13px] font-medium text-white/60 hover:text-white transition-colors duration-200 rounded-full hover:bg-white/[0.06]"
            >
              {link.label}
            </a>
          ))}
        ...
        <div className="hidden md:flex items-center gap-2 shrink-0">
          ...
          <a
            href="#contribute"
            className="flex items-center gap-1.5 h-[36px] px-4 bg-white hover:bg-white/90 text-black text-[12px] font-semibold tracking-[0.01em] rounded-full transition-all duration-200"
          >
            Contribute
